...
class WeatherLocationService:
    ...
    async def get_location() -> Optional[Dict[str, Any]]:
        """Get current location via MCP or fallback to IP-based location"""
        try:
            # Try MCP location server
            client = MCPClient(MCP_LOCATION_URL, "music-location-client")
                ...
                if item.get("type") == "text":
                    ...
                    if "Coordinates:" in text:
                        ...
                        if len(parts) >= 2:
                            ...
                            city = None
                            if "Location:" in text:
                                location_line = [line for line in text.split('\n') if 'Location:' in line][0]
                                city = location_line.split('Location:')[1].strip().split(',')[0]
            ...
            print(f"MCP location service unavailable: {e}")
        ...
        try:
            ...
            print(f"IP location fallback failed: {e}")
    ...
    async def get_weather(latitude: float, longitude: float) -> Optional[Dict[str, Any]]:
        """Get current weather via MCP or fallback to Open-Meteo API"""
        try:
            # Try MCP weather server
            client = MCPClient(MCP_WEATHER_URL, "music-weather-client")
            await client.initialize()
            ...
            # Parse weather from response
            content = result.get("content", [])
            for item in content:
                if item.get("type") == "text":
                    text = item.get("text", "")
                    weather_data = WeatherLocationService._parse_weather_text(text)
                    if weather_data:
                        weather_data["source"] = "mcp"
                        return weather_data
        except Exception as e:
            print(f"MCP weather service unavailable: {e}")
